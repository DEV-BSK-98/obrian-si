...
def credit_note_new (request):
    ...
    if request.method == "POST":
        credit_note = request.POST
        if not request.POST.get ("sale"):
            messages.error(request, f'Please Select The Invoice You want to DO a Credit Note On!')
            return render(request, "sales/credit-note-form.html", {"items": items, "invoices": invoices})
        if not config:
            messages.error(request, 'There Is No Configuration Setup Initialization')
            return render(request, "sales/credit-note-form.html", {"items": items, "invoices": invoices})
        tax_invoice = get_object_or_404 (Invoice, pk=credit_note.get ("sale"))
        if not tax_invoice:
            messages.error(request, 'Please Select a valid Invoice')
            return render(request, "sales/credit-note-form.html", {"items": items, "invoices": invoices})
        item_names = credit_note.getlist('itemName[]')
        qtys = credit_note.getlist('qty[]')
        prices = credit_note.getlist('price[]')
        tax_amount = credit_note.getlist('tax_amount[]')
        rates = credit_note.getlist('rate[]')
        amounts = credit_note.getlist('amount[]')
        discount_rate = credit_note.getlist('discount_rate[]')
        discount_amount = credit_note.getlist('discount_amount[]')
        amounts = credit_note.getlist('amount[]')
        inclusive_amounts = credit_note.getlist('inclusive_amount[]')
        ...
        if errors:
            ...
            return render(request, "sales/index.html", {"items": items})
        try:
            sale_ = Credit_Note_Invoice.objects.create(
                invoice=tax_invoice.id,
                tpin=config.tpin,
                bhfId=config.bhfId,
                orgInvcNo=credit_note.get('orgInvcNo', ''),
                cisInvcNo=credit_note.get('cisInvcNo', ''),
                custTpin=credit_note.get('custTpin', ''),
                custNm=credit_note.get('custNm', ''),
                salesTyCd="N",
                rcptTyCd="S",
                pmtTyCd=credit_note.get('pmtTyCd', '01'),
                salesSttsCd=credit_note.get('salesSttsCd', '01'),
                cfmDt=todaySi(),
                salesDt=todaySi(),
                cnclReqDt=todaySi(),
                totItemCnt=len(item_names),
                ...
                modrNm="Admin",
                saleCtyCd="1",
                currencyTyCd=credit_note.get ('currencyTyCd', "ZMW"),
                exchangeRt=credit_note.get ('exchangeRt', "1.0")
            )

            for itm in sale_items:
                item = Credit_Note_InvoiceItem.objects.create(**itm)
                sale_.itemList.add(item)

            sale_.save()
            return redirect('credit-note-info', sale_.id)
        except Exception as e:
            messages.error(request, f'There was an error saving the Credit Note: {e}')
            print(f"ERROR: {e}")
